...
class PinCodeTextField extends StatefulWidget {
  ...
  /// the style of the pasted text, default is
  /// [fontWeight: FontWeight.bold] while
  ...
  /// [AnimationType] for the text to appear in the pin code field.
  /// Default is [AnimationType.slide]
  ...
  /// Auto dispose the [controller] and [FocusNode] upon the destruction
  /// of widget from the widget tree. Default is [true]
  final bool autoDisposeControllers;

  /// Configures how the platform keyboard will
  /// select an uppercase or lowercase keyboard.
  /// Only supports text keyboards, other keyboard types will ignore
  /// this configuration. Capitalization is locale-aware.
...
class _PinCodeTextFieldState extends State<PinCodeTextField>
  ...
  void initState() {
    // if (!kReleaseMode) {
    //   print(
    //       "IF YOU WANT TO USE COLOR FILL FOR EACH
    // CELL THEN SET enableActiveFill = true");
    // }
    ...
    _inputList = List<String>.filled(widget.length, '', growable: true);
    // List<String>(widget.length);
    _initializeValues();
  ...
  void _assignController() {
    if (widget.controller == null) {
      _textEditingController =
          TextEditingController(text: widget.entryPincode ?? '');
    } else {
      ...
      if (widget.enabled && _inputList?.join('') != currentText) {
        if (currentText.length >= widget.length) {
          if (widget.onCompleted != null) {
            ...
            //  delay the onComplete event handler to give
            // the onChange event handler enough time to complete
  ...
  void dispose() {
    if (widget.autoDisposeControllers) {
      ...
      // if (!kReleaseMode) {
      //   print(
      //       "*** Disposing _textEditingController and _focusNode,
      //  To disable this feature please
      // set autoDisposeControllers = false***");
      // }
    }
...
// selects the right fill color for the field
  ...
  Widget buildChild(int index) {
    if (((_selectedIndex == index) ||
        ...
        _focusNode!.hasFocus &&
        widget.showCursor) {
      final cursorColor = widget.cursorColor ?? context.secondaryColor;
      final cursorHeight =
  ...
  Widget build(BuildContext context) {
    return SlideTransition(
      ...
      child: Container(
        // adding the extra space at the bottom to
        // show the error text from validator
            ...
            AbsorbPointer(
              child: AutofillGroup(
                child: TextFormField(
                  ...
                  // using same as background color so
                  // tha it can blend into the view
                  cursorWidth: 0.01,
                  ...
                  style: TextStyle(
                    ...
                    // it is a hidden textfield which should remain
                    // transparent and extremely small
                  ),
                ),
              ),
            ),
